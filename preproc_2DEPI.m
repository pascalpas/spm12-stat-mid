function preproc_2DEPI(subjectDir,subjectName,scanCode,anatomy,func,scanTR,mbFactor,sample_jobman)
% Input:
%   subjectDir:     directoryname of subject;
%   subjectName:    subject name;
%   scanCode:       consistent beginning of functional/anat filename
%   anatomy:        array with anatomical dir names
%   func:           array with functional dir names
%   scanTR:         TR of functionals (e.g. 1)
%   mbFactor:       multiband factor for slice timing  (e.g. 3)
%   sample_jobman   SPM12 batch preproc

% e.g.: preproc_2DEPI('..full/path/subject','Roger','sub','anat','func',3,2)

warning('off');

anatfile = dir(fullfile(subjectDir,anatomy,[scanCode,'*.nii']));
funcfile = dir(fullfile(subjectDir,func,[scanCode,'*.nii']));
hp_cutoff = 128;
...
TR = scanTR;
nSlices = size(y_b,3);
init_time = TR/(nSlices) + (TR/(nSlices)/2);

for nSlice = 1:(nSlices/mbFactor)
    time(nSlice) = init_time + (TR/(nSlices/mbFactor)) * (nSlice - 1);
end

slice_times = [];
for nReps = 1 : mbFactor
    slice_times =  [slice_times,time];
end
reference_time = time(round(length(time)/2));
...
eval(['[input_coreg_source,dirs]=spm_select(''FPList'',anatpath,''^',scanCode,'.*\.nii$'');']);
...
matlabbatch{4}.spm.spatial.normalise.estwrite.eoptions.tpm = cellstr(fullfile(batchesDir,'QC/ASSETS','TPM.nii'));
...
eval(['[t1img,dirs]=spm_select(''FPList'',anatpath,''^w',scanCode,'*'');']);
prefix = 'swra';
...
% Save figure
qcreport = fullfile(outputdir,['report.ps']);
...
for imap = 1:3
    ...
    jpegname = strcat(['map_',qctype,'_ax_func']);
    ...
    if imap == 1
        hs.string = ['Temporal Mean map for ',nicename];
    elseif imap == 2
        hs.string = ['Temporal Standard Deviation map for ',nicename];
    elseif imap == 3
        hs.string = ['Temporal Signal-Noise Ratio map for ',nicename];
    end
    
    titlebox = uicontrol('style','text');
    set(titlebox,'String',hs.string);
    titlebox.BackgroundColor = [1 1 1];
    
    sloverpos = num2cell(fig(1).Position);
    set(titlebox,'position',[0 (sloverpos{4}-(sloverpos{4}/25)) 670 20]);
    print('-append', '-dpsc2', qcreport);
...
for imap = 1:3
    ...
    jpegname = strcat(['map_',qctype,'_sag_func']);
    set(gcf,'PaperPositionMode','auto');
    print('-djpeg',jpegname);
